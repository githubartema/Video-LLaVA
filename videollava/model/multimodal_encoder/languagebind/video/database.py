...
class Database(object):
    ...
    def _get(self, key, fencode, fdecode):
        ...
        with self.database.begin() as txn:
            with txn.cursor() as cursor:
                if key in ("protocol", "shape", "dtype", "keys"):
                    fdecode = _pickle_decode
    ...
    def _fencodes(self, keys: list) -> list:
        """
    ...
    def _fdecodes(self, values: list) -> list:
        """
...
class ArrayDatabase(Database):
    ...
    def _fdecodes(self, values: list) -> ndarray:
        # def _fdecodes(self, values: list[bytes]) -> ndarray:
        # shape = (len(values),) + self.shape
        # return np.frombuffer(b"".join(values), dtype=self.dtype).reshape(shape)
        pass
...
class PILImageDatabase(Database):
    _dtype = None
    _shape = None
    ...
    def _fdecode(self, value: bytes) -> ndarray:
        ...
        # return np.asarray(Image.frombytes('RGB', self.shape, io.BytesIO(value)))
...
class TensorDatabase(ArrayDatabase):
    ...
    def _fdecodes(self, values: list) -> Tensor:
